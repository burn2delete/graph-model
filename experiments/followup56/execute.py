"""Canonical deterministic execution wrapper for GDM56.

GDM56 inherits the canonical GDM50 numerical repair, canonical GDM51 structured
learned heads, and the canonical GDM54/GDM55 arbitration/architecture line. Only
ambiguity-head relation-family curriculum balance/factorization changes.
"""
...
from experiments.followup55.execute import Encoder as GDM55Encoder
...
class Encoder(GDM55Encoder):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        head = dict(self.meta["head_training_reproducibility"])
        head["gdm56_execution_scope"] = "capability_and_ambiguity_heads"
...
def main():
    ...
    def deterministic_interop(_requested):
        if torch.get_num_interop_threads() != 1:
            raise RuntimeError("GDM56 reproducibility contract requires one inter-op thread")
        return None
