"""Canonical deterministic execution wrapper for GDM54.

GDM54 inherits the canonical GDM50 numerical repair and canonical GDM51 structured
learned heads. It changes only validation-only status arbitration over the existing
NONE and ambiguity evidence.
"""
...
SOURCE53 = "97b8ceb7c3820690545873e9ced2854ad1e26cee"
...
class Encoder(g50det.ReproducibleEncoder):
    def __init__(self, *args, **kwargs):
        ...
        head["gdm54_execution_scope"] = "capability_and_ambiguity_heads"
        head["canonical_gdm53_source_commit"] = SOURCE53
        head["gdm54_change_scope"] = "validation-only ambiguity-before-NONE status arbitration"
...
def main():
    ...
    def deterministic_interop(_requested):
        if torch.get_num_interop_threads() != 1:
            raise RuntimeError("GDM54 reproducibility contract requires one inter-op thread")
        return None
