...
SEED=24001;random.seed(SEED);torch.manual_seed(SEED);torch.set_num_threads(4)
...
def retrieve(qs,k=2):
 return torch.topk(F.normalize(emb(rt,rm,qs),dim=-1)@corpus.T,k,dim=1).indices.tolist()
# Mine Qwen top-2 hard negatives.
qs=[];targets=[]
for rep in range(16):
 for target in K:
  for phrase in TRAIN[target]:qs.append(f"Return the {phrase}.");targets.append(target)
ids=retrieve(qs,2);train=[]
for n,(q,target,ix) in enumerate(zip(qs,targets,ids)):
 cs=[K[i] for i in ix]
 if target not in cs:cs=[target,cs[0]]
 random.Random(SEED+n).shuffle(cs);train.append((q,cs,target))
# Frozen mmBERT + compact MLP reranker.
...
for ep in range(16):
 ...
 for q,cs,target in train:
  e=emb(dt,dm,[f"Request: {q} Candidate: {CAPS[c]}" for c in cs]);z=head(e).squeeze(-1);loss=F.cross_entropy(z[None,:],torch.tensor([cs.index(target)]));opt.zero_grad();loss.backward();opt.step()
rows=[(f"Return the {TEST[K[i%8]]}.",K[i%8]) for i in range(80)]
start=time.perf_counter();ids=retrieve([q for q,_ in rows],2);retr_ms=1000*(time.perf_counter()-start)/80;rec=sum(t in [K[i] for i in ix] for ix,(_,t) in zip(ids,rows))
...
rr=1000*(time.perf_counter()-start)/80
out={"reranker":"mmBERT-base","retrieval_recall_at_2":rec/80,"final_accuracy":correct/80,"retrieval_ms_per_world":retr_ms,"rerank_ms_per_world":rr,"total_ms_per_world":retr_ms+rr,"train_pairs":len(train),"trainable_parameters":sum(p.numel() for p in head.parameters()),"max_rss_kb":resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}
Path("artifacts").mkdir(exist_ok=True);Path("artifacts/gdm24.json").write_text(json.dumps(out,indent=2));print(json.dumps(out,indent=2))
