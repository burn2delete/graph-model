"""Independent evidence gate for GDM50."""
...
def holdout_detail(rs):
    by_clause=defaultdict(lambda:Counter(total=0,correct=0,accepted=0)); risk=defaultdict(lambda:Counter(total=0,correct=0)); conf=Counter()
    for r in rs:
        ref=r["reference"]; pred=r["prediction"]; jud=r["judgment"]
        n=max(1,len(pred.get("clause_evidence",[])) or len(pred.get("gate_evidence",{}).get("clauses",[])) or len(ref.get("paths",[])))
        if ref["status"]=="accepted":
            by_clause[n]["total"]+=1; by_clause[n]["correct"]+=int(bool(jud.get("request_correct"))); by_clause[n]["accepted"]+=int(pred["status"]=="accepted")
            if pred["status"]=="accepted":
                gold=set(ref.get("paths",[])); got=set(pred.get("selected",[]))
                for p in got-gold:
                    for g in (gold-got) or gold: conf[(g,p)]+=1
        else:
            risk[ref["status"]]["total"]+=1; risk[ref["status"]]["correct"]+=int(pred["status"]==ref["status"])
    return {"answerable_by_clause_count":{str(k):dict(v) for k,v in sorted(by_clause.items())},"risk_by_reference_status":{k:dict(v) for k,v in sorted(risk.items())},"semantic_confusions":[{"reference":a,"predicted":b,"count":c} for (a,b),c in conf.most_common(12)]}
...
def verify_summary(path):
    ...
    if tr.get("gdm50_family")!=cfg.get("family"): errors.append("training family mismatch")
    caldoc=load_json(directory/"calibration.json") if (directory/"calibration.json").exists() else {}; cal=caldoc.get("selected",{})
    if cfg.get("family")=="explicit-none-listwise":
        if tr.get("risk_training_kind")!="explicit NONE candidate learned jointly with listwise capability scores; status thresholds validation-calibrated": errors.append("null risk kind mismatch")
        if cal.get("mode")!="explicit-none-margins" or cal.get("selection_split")!="validation": errors.append("null calibration mismatch")
        if cal.get("calibration_scope")!="expanded-validation-only": errors.append("null calibration scope mismatch")
        for key in ("none_margin_threshold","ambiguity_margin_threshold"):
            if not isinstance(cal.get(key),(int,float)): errors.append("invalid "+key)
        if arm_name=="null-argmax" and abs(float(cal.get("none_margin_threshold",99)))>1e-12: errors.append("argmax NONE threshold not fixed at zero")
    else:
        if tr.get("risk_training_kind")!="validation-only deterministic clause thresholds": errors.append("control risk kind mismatch")
        if cal.get("mode")!="clause-thresholds" or cal.get("gate")!="clause-hybrid" or cal.get("selection_split")!="validation": errors.append("control calibration mismatch")
    ...
    for split,count in (("regression",s.get("regression_examples")),("holdout",s.get("secondary_holdout_examples"))):
        ...
        for r in pr:
            ...
            if cfg.get("family")=="explicit-none-listwise":
                ...
                if not isinstance(ev,list) or not ev or any(not all(k in e for k in ("best_real_id","none_margin","real_margin")) for e in ev): errors.append("missing explicit-NONE evidence"); break
            elif "gate_evidence" not in r["prediction"]: errors.append("control lacks gate evidence"); break
        if pr:
...
def main():
    p=argparse.ArgumentParser(); p.add_argument("root"); p.add_argument("--seeds",default="5001,5002"); p.add_argument("--tasks",default="operation,schema"); a=p.parse_args(); root=Path(a.root); seeds=[int(x) for x in a.seeds.split(",") if x]; tasks=[x for x in a.tasks.split(",") if x]
    ...
    for key,s in summaries.items():
        task,arm,backbone,seed=key; g=groups.setdefault((task,arm,backbone),{"reg":[],"hold":[],"p50":[],"p95":[],"rss":[],"recall":[],"precision":[],"clause":defaultdict(Counter),"risk":defaultdict(Counter),"conf":Counter()}); g["reg"].append(derived(metrics_for(s,"regression_metrics",task))); g["hold"].append(derived(metrics_for(s,"secondary_holdout_metrics",task))); g["p50"].append(s["generation_latency"]["p50_ms"]); g["p95"].append(s["generation_latency"]["p95_ms"]); g["rss"].append(s["memory"].get("rss_after_evaluation_kib")); cm=s["secondary_holdout_clause_metrics"]
    ...
    report={"format":"gdm50-batch-report-v1","source_commit":os.environ.get("GITHUB_SHA","unrecorded"),"complete":complete,"verified":complete,"expected_results":len(expected),"verified_results":len(summaries),"missing_results":missing,"unexpected_results":unexpected,"failed_results":failures,"dataset_hashes":dataset_hashes,"secondary_holdout_hashes":holdout_hashes,"aggregates":ag,"hypotheses":["GDM49 binary matching loses recall because every clause must independently exceed an absolute threshold.","An explicit learned NONE candidate can give listwise scoring an open-set outcome without abandoning its stronger relative capability discrimination.","A calibrated NONE-vs-best-real margin can trade answerable recall against NO_MATCH publication risk without compounding candidate-wise thresholds.","Contrastive listwise supervision may reduce persistent adjacent semantic confusions while preserving the explicit NONE mechanism."],"interpretation_limits":["Frozen DistilBERT with learned feature-space listwise head; no transformer fine-tuning.","Explicit NONE is a learned feature-space candidate and risk thresholds are validation calibrated, not a request-global trained status head.","Schema generation remains catalog projection plus deterministic SDL/Federation realization, not unconstrained schema invention.","The Subscription/Claim secondary holdout is synthetic and not enterprise OOD."]}
